...
export function WithdrawModal({ open, onOpenChange, bucketId, bucketName }: WithdrawModalProps) {
  ...
  const { updateBucketBalance, getBucket } = useBuckets()
  ...
  const handleWithdraw = () => {
    ...
    setTimeout(() => {
      ...
      updateBucketBalance(bucketId, -numAmount)
      setStep("success")
      toast({
        title: "Withdrawal Complete",
        description: `Successfully withdrawn $${numAmount.toLocaleString()} to your wallet.`,
      })
    }, 2000)
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="glass border-purple-500/20 sm:max-w-md bg-black/90 backdrop-blur-2xl">
        ...
        {step === "amount" && (
          <DialogFooter>
            <Button
              disabled={!amount || Number(amount) <= 0 || Number(amount) > (bucket?.balance || 0)}
