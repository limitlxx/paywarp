...
export function TransferModal({ open, onOpenChange, initialFromId }: TransferModalProps) {
  ...
  const { transferBetweenBuckets, buckets, getBucket } = useBuckets()
  ...
  const handleTransfer = () => {
    ...
    setTimeout(() => {
      ...
      transferBetweenBuckets(fromId, toId, numAmount)
      setStep("success")
      toast({
        title: "Transfer Complete",
        description: `Successfully moved $${numAmount.toLocaleString()} from ${fromBucket?.name} to ${toBucket?.name}.`,
      })
    }, 2000)
  }
